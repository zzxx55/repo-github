    ...
    for (let y = 0; y < matrix.length; y++) {// на всю длинну массива создаем 
        ...
        for (let x = 0; x < matrix[y].length; x++) {// идем по всем Х
            ...
            if (cell.flag) { // при создание проверяем на флаг 
                imgElement.src = 'pikch/flag.png'
                continue
            }

            if (cell.paten) {//на тех клетках где есть потесиал отображать серые квадраты
                imgElement.src = 'pikch/paten.png'
                continue
            }
            if (!cell.show) {// на то видна клетка или нет
                imgElement.src = 'pikch/none.png'
                continue
            }

            if (cell.mine) {// есть мина или нет
                imgElement.src = 'pikch/mine.png'
                continue
            }

            if (cell.number) {// есть ли число 
                imgElement.src = 'pikch/number' + cell.number + '.png'
                continue
            }
            imgElement.scr = 'pikch/free.png'// если нет нечего пустое поле 
...
function countTime(null_min, null_sec) { // возникла проблема, что обнулять получается только обновлением страницы
